...
**For mind-palace apps, use `bun gen:app`** — the TurboRepo generator at `turbo/generators/` mirrors `apps/web/` (with `usePixiApp`, Zod-validated props, `atomWithIDB`, the four Pillars wired). This skill documents the upstream `create-pixi` CLI for **standalone projects outside mind-palace**, or for understanding what create-pixi produces.
