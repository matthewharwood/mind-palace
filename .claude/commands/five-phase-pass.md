---
description: Propagate a monorepo change through the mind-palace five-phase pass (P1 apps/web → P2 generator template → P3 regenerate test-project + gate → P4 docs/skill → P5 cross-skill audit). Pass `--baseline` to run P5-only against the cumulative current stack.
...
// h1-wait is the contract: every mind-palace route renders an h1.
...
| `fallow-ignore-next-line\s+circular-dependency` *(suppression of the circular-deps rule)* | A circular import is a real bug surface (initialization-order issues, harder reasoning). Suppressing it doesn't fix it. The canonical refactor is to extract the shared symbol(s) into a leaf module and have BOTH original modules depend on the leaf — see CLAUDE.md "No circular imports" rule and `attack-fx/textures.ts` for the worked example. | OK in `fallow/SKILL.md` "anti-pattern" callouts; OK in this command's docs |
