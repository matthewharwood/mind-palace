---
description: Kill any stale dev/preview/storybook server processes and free their ports. Run before `bun run check` or `/five-phase-pass` to eliminate the documented parallel-worker race against a zombie Vite/Storybook process — the recurring cause of intermittent `ERR_CONNECTION_REFUSED` on Playwright gate runs.
---
