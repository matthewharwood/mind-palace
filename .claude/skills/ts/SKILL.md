...
`isolatedDeclarations: true` is the eventual target but is left **off** in the base today: the t3-env `createEnv` return type is generic-heavy and not yet annotated for fast .d.ts emit. Turn it on once the env wiring is annotated and surface the change in CLAUDE.md.
...
```
`as` casts at module boundaries are forbidden by CLAUDE.md. Parse with Zod instead — see `zod` for the schema authoring rules.
